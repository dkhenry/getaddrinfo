...
unsafe fn ifa_netmask_to_prefix(netmask: libc::sockaddr) -> u8 {
    match netmask.sa_family as i32 {
        libc::AF_INET => {
            let addr: libc::sockaddr_in = mem::transmute::<libc::sockaddr,libc::sockaddr_in>(netmask);
            mem::transmute::<libc::in_addr,u32>(addr.sin_addr).count_ones() as u8
        },
        libc::AF_INET6 => {
            let addr: sockaddr_in6 = *(mem::transmute::<*const libc::sockaddr,*const libc::sockaddr_in6>(&netmask));
...
unsafe fn sockaddr_to_addr( sock: libc::sockaddr ) -> [u32; 4] {
    ...
    match sock.sa_family as i32 {
        ...
        libc::AF_INET6 => {
            ...
            ptr::copy(&mem::transmute::<libc::in6_addr,[u32;4]>(addr.sin6_addr),&mut rvalue,4);
            rvalue
        },
        _ => rvalue
    }
